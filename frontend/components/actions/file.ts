...
export async function deleteUploadedFile(fid: number) {
  ...
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_BACKEND}/api/document/documents/file-delete/${fid}/`,
      {
        method: 'DELETE',
        headers: {
...
export async function downloadPdf(fid: number) {
  const access = await getAccess();
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND}/api/document/documents/pdf-download/${fid}/`, {
      method: "GET",
      headers: {
        "Authorization": `Bearer ${access}`
      }
...
export async function uploadFile({formData, kind = 'pdf'}: {formData: FormData, kind?: string}) {
  ...
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND}/api/document/documents/${kind}-upload/`, {
      method: "POST",
      body: formData,
      headers: {
        "Authorization": `Bearer ${access}`
...
export async function getFileUrls(fid: number) {
  ...
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_BACKEND}/api/document/documents/file-urls/${fid}/`,
      {
        headers: {
          Authorization: `Bearer ${access}`
        },
